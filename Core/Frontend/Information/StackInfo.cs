...
public static class Stack
{
    public static TypedValue GetArgument(ScratchVariable variable)
    {
        ...
        return new(
            ...
            procedure.Arguments[variable.Name], new() {{"ARGUMENT_NAME", variable.Name}});
    }

    public static string PushArgument(object expression) => $"push {ScratchScriptVisitor.StackName} {expression.Format()}\n";

    public static string PopArguments() => $"repeat {ScratchScriptVisitor.Instance.Procedures.Last().Arguments.Count}\n{ScratchScriptVisitor.PopStackCommand}\nend";
}